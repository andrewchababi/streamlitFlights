...
def flights_left(df):
    return len(df[df['Status'] == 'On time'] + df[df['Status'] == 'Delayed'])
...
def prep_closing_time(df):
    ...
    last_flight['updatedTime'] = pd.to_datetime(last_flight['updatedTime'], errors='coerce').dt.round('h')

    last_time = last_flight['updatedTime'].iloc[0]
